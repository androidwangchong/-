...
public class OptionsActivity extends BaseActivity {
			...
			scaleScreenButton
					.setOnCheckedChangeListener(new OnCheckedChangeListener() {
						...
						public void onCheckedChanged(CompoundButton buttonView,
							...
							boolean rs = GameConfig.persistConfigScaleScreen(
	...
	private void sendNewLevelBackToPuzzleView() {
		...
		intent.putExtra(PuzzleImgActivity.Extra_COLUMNCOUNT,
				columnSpinner.getSelectedItemPosition()
						+ GameConfig.MINCOLUMNCOUNT);
		intent.putExtra(PuzzleImgActivity.Extra_ROWCOUNT,
				rowSpinner.getSelectedItemPosition()
						+ GameConfig.MINCOLUMNCOUNT);
	...
	private void initValues() {
		columnSpinner.setSelection(assign_columncount
				- GameConfig.MINCOLUMNCOUNT);
	...
	private void initComponent() {
		...
		if (extras != null
				&& extras.get(PuzzleImgActivity.FROM_PUZZLEVIEW_VAR) != null) {
			this.fromPuzzleView = true;
			assign_rowcount = extras.getInt(PuzzleImgActivity.Extra_ROWCOUNT);
			assign_columncount = extras
					.getInt(PuzzleImgActivity.Extra_COLUMNCOUNT);
		}
	...
	void showToast(boolean success) {
		if (success)
			Toast.makeText(this, R.string.optiontsSaveSuccessfullLabel,
					Toast.LENGTH_SHORT).show();
		else {
			Toast.makeText(this, R.string.optionsSaveFailedLabel,
					Toast.LENGTH_SHORT).show();
		}
	}
	...
	class RowOrcolumnCountSelectedAction implements OnItemSelectedListener {
		...
		public void onItemSelected(AdapterView<?> parent, View view,
			...
			if (parent == columnSpinner) {
				...
				boolean rs = GameConfig.persistConfigColumnCount(
						getSharedPreferences(GameConfig.CONFIG_FILENAME, 0),
						position + GameConfig.MINCOLUMNCOUNT);
				showToast(rs);
			} else {
				...
				boolean rs = GameConfig.persistConfigRowCount(
						getSharedPreferences(GameConfig.CONFIG_FILENAME, 0),
						position + GameConfig.MINROWCOUNT);
				showToast(rs);
			}
		}
